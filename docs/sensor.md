...
- `value_type` controls how the value should be rendered in a human readable representation. The attribute may have may have the values `percent`, `temperature`, `illuminance`, `speed_ms` or `current`. Required.
- `name` is the name of the object. Defaults to `"Sensor"` and the value type.
