...
- Add `ExpectedResponse` to `xknx.telegram.apci` - a mixin marking that an APCI request service (`DeviceDescriptorRead`, `AuthorizeRequest`, `PropertyValueRead`) has a KNX-spec-defined response type, set once on the class as `RESPONSE_TYPE`. `P2PConnection.request()` no longer takes an `expected=` argument - it infers and verifies the expected response from `type(payload).RESPONSE_TYPE` and returns the correspondingly typed `Telegram[ResponseType]`, so procedures no longer need `assert isinstance(response.payload, ResponseType)` (or even a `None` check) to get a typed `.payload`.
