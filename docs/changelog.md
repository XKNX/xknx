...
- Add `Management.request_broadcast()` - broadcasts a request service and yields the responses to it, eg. `async for telegram in xknx.management.request_broadcast(apci.IndividualAddressRead(), timeout=3)`. It replaces the `broadcast()` context, `send_broadcast()` and `receive()` trio for the common case, listening for the response type the payload declares and narrowing each telegram to it. Any number of devices may answer, so it yields until `timeout` elapses rather than returning a single telegram like `P2PConnection.request()`.
