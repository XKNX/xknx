...
- Add `dmp_interface_object_read_r` (KNX 03.05.02 §3.27.2), `dmp_interface_object_write_r` (§3.25.2), `dmp_interface_object_scan_r` (§3.28.2), and `dmp_interface_object_verify_r` (§3.26.2) to the `device/` procedure package. All procedures take a `P2PConnection` directly. Read, write, and verify handle chunking transparently for payloads exceeding 15 elements (4-bit `nr_of_elem` limit); verify compares per block as required by the spec. Scan enumerates all interface objects and their property descriptions via `PropertyDescriptionRead`/`PropertyValueRead` loops, returning a list of `ScannedInterfaceObject`.
