...
- Add `dmp_authorize_r_co(conn, key)` and `dmp_authorize2_r_co(conn, client_key)` to `xknx.management.procedures` — KNX v02.01.02 - Management Procedures 03.05.02 - §3.5.1/§3.5.2 DM_Authorize/DM_Authorize2, obtaining access authorization on an already-open `P2PConnection`. `dmp_authorize2_r_co` authorizes with `FREE_ACCESS_KEY` first, then `client_key`, and keeps whichever gives the better access level, for callers that don't know upfront whether a device is already locked with `client_key`. `dmp_authorize_r_co` has no default for `key`: per §3.5.1 the exchange only happens when authorization is required at all, so sending it unconditionally with `FREE_ACCESS_KEY` risks a timeout against a device that doesn't implement A_Authorize. Also add `dmp_connect_r_co(conn)` — §3.2.1 DMP_Connect_RCo — which reads Device Descriptor Type 0 on an already-open connection to confirm it's live and raises if the response isn't type 0, returning the mask version. Like `dm_restart_r_co`/`nm_individual_address_check_conn`, none of these has a `(xknx, ...)` counterpart that opens its own connection: an authorization or connection check only has meaning for the lifetime of the connection it ran on, so they're meant to be composed with other procedures under a caller-managed connection.
- Add `dmp_interface_object_read_r`, `dmp_interface_object_write_r`, `dmp_interface_object_scan_r` and `dmp_interface_object_verify_r` to `xknx.management.procedures.device` — KNX v02.01.02 - Management Procedures 03.05.02 - §3.27.2/§3.25.2/§3.28.2/§3.26.2, reading, writing, discovering and verifying Properties of a device's Interface Objects on an already-open `P2PConnection`. Read, write and verify transparently chunk requests exceeding the 4-bit `nr_of_elem` field's 15-element limit (KNX v02.01.01 - Application Layer 03.03.07 - §3.4.4) into multiple `A_PropertyValue_Read`/`_Write` exchanges, verify comparing per chunk as the spec requires; scan enumerates every interface object and its property descriptions via `A_PropertyDescription_Read`/`A_PropertyValue_Read`, returning a list of `ScannedInterfaceObject`.

### Internals

- The connection heartbeat and the sequence counter handling for received frames are extracted into `ConnectionHeartbeat` and `IncomingSequenceCounter` in `xknx.io.data_connection` (KNX v01.06.02 - Core 03.08.02 - §5 "Communication Channels"), shared by the tunnel and the device management connection instead of being duplicated. The mechanics are unchanged; what a failure means stays with the owner - a tunnel reconnects and schedules a disconnect for out-of-order frames, a device management connection closes and only observes.
