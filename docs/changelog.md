...
- `apci.FunctionPropertyCommand` is an `APCIRequest[FunctionPropertyStateResponse]` now, like `FunctionPropertyStateRead` already was. It carries a KNX-spec-defined response (KNX v02.01.02 - Management Procedures 03.05.02 - §3.30.1 DM_FunctionProperty_Write_R) and was missed when the other point-to-point request services were converted, so `P2PConnection.request()` sent it without verifying or narrowing the response type.
- `apci.FunctionPropertyExtCommand` is an `APCIRequest[FunctionPropertyExtStateResponse]` now, like `FunctionPropertyExtStateRead` already was - the same fix as `FunctionPropertyCommand` above, for the extended Function Property family (KNX v02.01.02 - Management Procedures 03.05.02 - §3.30.2 DMP_ExtFunctionProperty_Write_R).
- `apci.FunctionPropertyCommand.to_knx()` raises `ConversionError` for an `object_index`/`property_id` outside 0-255/1-255 now, like `FunctionPropertyExtCommand` already does for its own fields via `_pack_function_property_ext_header()`. It previously packed them with a bare `struct.pack("!BB...")`, so an out-of-range value surfaced as a raw `struct.error` instead. `property_id` has no default anymore - the guard requires at least 1, so the old default of 0 could be constructed but never serialized without raising.
- `apci.LinkWrite` is an `APCIRequest[LinkResponse]` now - the same fix as `PropertyValueWrite`/`FunctionPropertyCommand` above, for `A_Link_Write` (KNX v02.01.01 - Application Layer 03.03.07 - §3.4.6.2), which was also missed when the other point-to-point request services were converted.
...
- Add `dm_function_property_write_r` and `dm_function_property_write_r_conn` to `xknx.management.procedures` — KNX v02.01.02 - Management Procedures 03.05.02 - §3.30.1 DM_FunctionProperty_Write_R, invoking a Function Property on an interface object (`A_FunctionPropertyCommand`, confirmed by `A_FunctionPropertyState_Response`). The spec leaves the meaning of the response's `return_code` and its error handling to whichever Configuration Procedure calls this one, so the full response is returned rather than raised on a non-zero code. There is no separate DM_/DMP_ procedure to read a (non-extended) Function Property's state without invoking a command - send `apci.FunctionPropertyStateRead` directly for that. Unlike the Property/Memory procedures, an over-long `command` is rejected upfront via `max_apdu_length` rather than chunked - the spec describes no way to split a Function Property command across multiple PDUs. `max_apdu_length` itself is validated to be positive, like the Property/Memory procedures already do.
- Add `dmp_ext_function_property_write_r` and `dmp_ext_function_property_write_r_conn` to `xknx.management.procedures` — KNX v02.01.02 - Management Procedures 03.05.02 - §3.30.2 DMP_ExtFunctionProperty_Write_R, the extended-addressing sibling of `dm_function_property_write_r` (`A_FunctionPropertyExtCommand`/`A_FunctionPropertyExtState_Response`, addressing by Interface Object Type + Object Instance instead of a connection-local object index). Same non-raising `return_code` handling and `max_apdu_length` bound (including its positivity check) as the base procedure.
- Add `dmp_load_state_machine_write_r_co_io` to `xknx.management.procedures` — KNX v02.01.02 - Management Procedures 03.05.02 - §3.31.3 DMP_LoadStateMachineWrite_RCo_IO, writing a load event to an interface object's Load State Machine (`PID_LOAD_STATE_CONTROL`, `A_PropertyValue_Write`/`_Response`) and returning the resulting `LoadState` (KNX v01.10.01 - Resources 03.05.01 - §4.23.2.3.1). An optional `expected_state` polls (re-reading the property) until it is reached or `poll_timeout` elapses - 30 seconds by default, the spec's own minimum wait before a state transition may be considered failed (§4.23.2.1) - raising if the machine reports `ERROR` instead; an illegal or unknown event is otherwise ignored by the device without a change of state (§4.23.2.3.2), so `expected_state` is the only way to detect that. The response is checked to be exactly 1 octet before decoding it as a state, so a device that misbehaves and echoes back the 10 octet write event can't have its event-type byte misread as a state value. Also export `LoadState`, `SegmentType`, `LOAD_EVENT_SIZE` and the load event builders (`start_loading`, `load_completed`, `unload`, `no_operation`, `alloc_abs_data_seg`, `alloc_abs_stack_seg`, `alloc_abs_task_seg`, `task_ptr`, `task_ctrl_1`, `task_ctrl_2`, `relative_allocation`, `data_relative_allocation`, §3.31.3.1-4) from `xknx.management.procedures` alongside the procedure itself - they're its entire argument vocabulary. Each builder now validates its own fields and raises `ValueError` for an out-of-range one, rather than silently truncating via `& 0xFF` or raising a bare `OverflowError` from `int.to_bytes()`.
- Add `dmp_load_state_machine_read_r_io` and `dmp_load_state_machine_verify_r_io` to `xknx.management.procedures` — KNX v02.01.02 - Management Procedures 03.05.02 - §3.33.3/§3.32.3, reading and verifying an interface object's Load State Machine (`PID_LOAD_STATE_CONTROL`, `A_PropertyValue_Read`/`_Response`) without writing anything first, same decoding as `dmp_load_state_machine_write_r_co_io`'s own response handling. `verify_r_io` raises the existing `VerificationError` on a mismatch. Add their extended-addressing siblings `dmp_ext_load_state_machine_read_r_co_io` and `dmp_ext_load_state_machine_verify_r_co_io` — §3.33.4/§3.32.4, addressing by Interface Object Type + Object Instance via `A_FunctionPropertyExtState_Read`/`_Response` instead. Unlike `dmp_ext_function_property_write_r`'s own `return_code` (Function Property specific, left to the caller), `PID_LOAD_STATE_CONTROL` is a `PDT_CONTROL` property, for which the Application Layer defines a fixed response shape (KNX v02.01.01 - Application Layer 03.03.07 - §3.4.8.4): a positive `return_code` always carries the 1 octet state, a negative one always carries none - so a negative `return_code` is unambiguously an error here and is raised as `ManagementConnectionError` rather than returned.
- Add `dmp_run_state_machine_write_r_io`, `dmp_run_state_machine_verify_r_io` and `dmp_run_state_machine_read_r_io` to `xknx.management.procedures` — KNX v02.01.02 - Management Procedures 03.05.02 - §3.34.3/§3.35.3/§3.36.3, the Run State Machine counterpart of the Load State Machine family: writing a run event to `PID_RUN_STATE_CONTROL` and reading/verifying its resulting `RunState` (KNX v01.10.01 - Resources 03.05.01 - §4.24.2.3.1), via the new `run_state` module's `restart`/`stop`/`no_operation` event builders. Unlike `load_state`'s dozen builders, these three are not re-exported from `xknx.management.procedures` - `no_operation` would collide with `load_state.no_operation`, and `restart`/`stop` read confusingly next to the unrelated `dm_restart`/`dm_restart_r_co` device-restart procedures - import `xknx.management.procedures.device.run_state` instead. `write_r_io`'s optional `expected_state` polls the same way the Load State write does; there is no `RunState.ERROR` to short-circuit on, since the Run State Machine has none. Add the extended-addressing siblings `dmp_ext_run_state_machine_write_r_co_io`, `dmp_ext_run_state_machine_verify_r_co_io` and `dmp_ext_run_state_machine_read_r_co_io` — §3.34.4/§3.35.4/§3.36.4, same non-returning `return_code` handling as the Load State Ext family. The write variant needs a `max_apdu_length` of at least 16 to succeed at all: its 6 octet `A_FunctionPropertyExtCommand` header plus the fixed 10 octet run event is one octet past an L_Data_Standard frame's 15 octet default, so it must be called with a device's real (larger) `PID_MAX_APDU_LENGTH`.
- Add `dmp_download_loadable_part_r_co_io` to `xknx.management.procedures` — KNX v02.01.02 - Management Procedures 03.05.02 - §3.31.4 DMP_DownloadLoadablePart_RCo_IO, downloading one loadable part into an interface object: Unload (polled up to 30 seconds for `LoadState.UNLOADED`, the spec's own "Retry read Property for 30 Seconds"), Start Loading, any number of caller-supplied Additional Load Control events (built with `load_state.alloc_abs_data_seg` and siblings), then Load Completed - each of the latter three checked once against the state the spec mandates for it and raising immediately rather than retrying. The step the spec itself leaves unspecified - "Load the loadable data via Property access or memory access" - is a caller-supplied `load_data` callback rather than a guessed mechanism, since which one a given device expects isn't something this procedure can know; typically a call to `dmp_interface_object_write_r` or `dmp_mem_write_r_co` against the same connection.
- Add `dm_group_object_link_read_r_cl` and `dm_group_object_link_write_r_cl` to `xknx.management.procedures` — KNX v02.01.02 - Management Procedures 03.05.02 - §3.37.2/§3.37.3, reading and writing the Group Address(es) linked to a Group Object via `A_Link_Read`/`A_Link_Write` (KNX v02.01.01 - Application Layer 03.03.07 - §3.4.6.1/.2). `read_r_cl` chunks in groups of up to 6, continuing at start_index 1, 7, 13 as long as a response is full, and raises if a Group Object has more than 18 linked Group Addresses - past what the service's 4 bit `start_index` field can address at all - rather than silently returning a truncated list. A negative `A_Link_Response` (empty list, start_index=0) ends `read_r_cl`'s loop without raising - §3.37.1 uses the same response for "Group Object doesn't exist" and "no more addresses from here", which it does not itself distinguish - but is unambiguously an error for `write_r_cl`, which never chunks and raises `ManagementConnectionError` on it. Both return the new `GroupObjectLink` (`sending_address`, `group_addresses`).
- Add `dmp_prog_mode_switch_r_co` to `xknx.management.procedures` — KNX v02.01.02 - Management Procedures 03.05.02 - §3.13.2 DMP_ProgModeSwitch_RCo, switching a device's Programming Mode on or off via a read-modify-write of the single octet `curr_prog_mode` at memory address 0060h (KNX v01.10.01 - Resources 03.05.01 - §4.26.3 "Programming Mode - Realisation Type 2"). Bit 0 carries the mode; bit 7 (`p_parity`) is recomputed from scratch as the true even parity of bits 0-6 on every call - matching calimero-core's `ManagementProceduresImpl.setProgrammingMode()` - rather than toggled, since §4.26.3.1 defines `p_parity` as "the parity bit of the complete octet" unconditionally; this also repairs an already-invalid parity bit even when the requested mode matches the device's current state.
- Add `dmp_mem_read_extended_r`, `dmp_mem_write_extended_r` and `dmp_mem_verify_extended_r` to `xknx.management.procedures` — KNX v02.01.02 - Management Procedures 03.05.02 - §3.24/§3.22/§3.23, the extended-addressing (24 bit, 16 MiB) siblings of `dmp_mem_read_r_co`/`dmp_mem_write_r_co`/`dmp_mem_verify_r_co` via `A_MemoryExtended_Read`/`_Write` (KNX v02.01.01 - Application Layer 03.03.07 - §3.4.9.1/.2). Unlike the non-extended `A_Memory_Write`, `A_MemoryExtended_Write` "shall be a confirmed service" - its response's `return_code` is checked and raised on for every chunk of both read and write, so `write_extended_r` needs no `verify`/`write_delay` of its own. A device may confirm a write with the positive `E_SUCCESS_WITH_CRC` return code (§3.4.9.2.1, Table 4) instead of a bare `E_SUCCESS`, carrying a CRC16-CCITT the spec explicitly makes optional and device-discretionary - `write_extended_r` accepts either as success without verifying the CRC itself; use `verify_extended_r` for an explicit read-back comparison.
- Add `dm_identify_r` and `dm_identify_r_co2` to `xknx.management.procedures` — KNX v02.01.02 - Management Procedures 03.05.02 - §3.4.2/§3.4.3, identifying a device. `dm_identify_r` reuses `dmp_connect_r_co` for its own Device Descriptor Type 0 read (step 1), then reads `PID_MGT_DESCRIPTOR_01` (step 2) only for devices reporting DD0 = 0300h, returning the new `IdentifiedDevice`. `dm_identify_r_co2` takes the DD0 already obtained from the connection's own `DM_Connect_RCo` (per the spec's own note that it "already returns" this value) and reads `PID_MANUFACTURER_ID`/`PID_HARDWARE_TYPE`, returning the new `DeviceIdentity`. Neither implements §3.4.3's own "repeated up to three times" retry-on-failure clause - the spec doesn't say whether that means three attempts total or three retries after the first failure, and no other procedure in this package has an established retry convention to extend; a caller wanting that behaviour can wrap the call in its own retry loop.

### Internals

- Add `xknx.exceptions.VerificationError`, a new parent of `PropertyVerificationError` for "the device's read-back data doesn't match what was expected" independent of which service produced it. `dmp_mem_write_r_co`/`dmp_user_mem_write_r_co`'s `verify` and `dmp_mem_verify_r_co`/`dmp_user_mem_verify_r_co` raise it directly, so a caller can catch `VerificationError` uniformly across Property and Memory operations instead of `PropertyVerificationError` only matching the former. Non-breaking: existing code catching `PropertyVerificationError` or `ManagementConnectionError` is unaffected.
