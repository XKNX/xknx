...
- `apci.FunctionPropertyCommand.to_knx()` raises `ConversionError` for an `object_index`/`property_id` outside 0-255/1-255 now, like `FunctionPropertyExtCommand` already does for its own fields via `_pack_function_property_ext_header()`. It previously packed them with a bare `struct.pack("!BB...")`, so an out-of-range value surfaced as a raw `struct.error` instead.
