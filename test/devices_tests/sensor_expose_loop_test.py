...
class TestSensorExposeLoop:
    ...
    async def test_array_sensor_loop(
        ...
        expose = ExposeSensor(
            xknx, value_type=value_type, name="TestExpose", group_address="1/1/1"
        )
        ...
        sensor = Sensor(
            xknx, value_type=value_type, name="TestSensor", group_address_state="1/1/1"
        )
...
class TestBinarySensorExposeLoop:
    ...
    async def test_binary_sensor_loop(
        ...
        bin_sensor = BinarySensor(xknx, name="TestSensor", group_address_state="1/1/1")
...
class TestBinarySensorInternalGroupAddressExposeLoop:
    ...
    async def test_binary_sensor_loop(
        ...
        expose = ExposeSensor(
            xknx, value_type=value_type, name="TestExpose", group_address="i-test"
        )
        xknx.devices.async_add(expose)
        assert expose.resolve_state() is None
        ...
        bin_sensor = BinarySensor(xknx, name="TestSensor", group_address_state="i-test")
        xknx.devices.async_add(bin_sensor)
        assert bin_sensor.state is None
