        ...
        [
            ...
            (
                ...
                [
                    ...
                    + b"\x29\x01\x02\x03"
                    + 49 * b"\x00",
                    b"\x01\x26\x02" + b"\x01\x02" + 59 * b"\x00",
                ],
                True,
            ),
            (
                [
                    ...
                    + b"\x29\x01\x02\x03"
                    + 49 * b"\x00",
                    b"\x01\x24\x3d" + 61 * b"\x01",
                    b"\x01\x36\x02" + b"\x02" + 60 * b"\x00",
                ],
                True,
            ),
        ],
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ),  # subtract the KNX USB Transfer Protocol Header (only in start packet!)
        ...
        ),  # subtract 52 (64 - 3 - 8 - 1) for the payload size fo the first frame
...
def test_get_packet_type(
    ...
    """ """
    assert expected == get_packet_type(
        overall_length, remaining_length, sequence_number
    )
...
class TestKNXToUSBHIDConverter:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                # test start and partial, partial and end
                b"\x03" + b"\x01\x02\x03" + 48 * b"\x00" + b"\x01" + b"\x0f",
                2,
                True,
            ),
            (
    ...
    def test_initialization(self, data: bytes, frame_count: int, valid_frame: bool):
        ...
        for index, frame in enumerate(frames):
            ...
            if (
                frame.report_header.packet_info.sequence_number
                == SequenceNumber.FIRST_PACKET
            ):
                assert frame.report_body.transfer_protocol_body.data.startswith(
                    data[:53]
                )
            else:
