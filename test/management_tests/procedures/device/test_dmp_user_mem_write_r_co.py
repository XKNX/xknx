...
async def test_dmp_user_mem_write_r_co_max_apdu_length_not_positive(
    xknx_setup: XKNX,
) -> None:
    """Test dmp_user_mem_write_r_co raises ValueError for max_apdu_length <= 0."""
    xknx = xknx_setup
    ia = IndividualAddress("4.0.10")
    ...
    with pytest.raises(ValueError, match=r"max_apdu_length must be positive, got 0"):
        await dmp_user_mem_write_r_co(
            conn, address=0x1000, data=b"\x01", max_apdu_length=0
        )
    await conn.disconnect()


async def test_dmp_user_mem_write_r_co_no_room_for_data(xknx_setup: XKNX) -> None:
    """Test dmp_user_mem_write_r_co raises ValueError when max_apdu_length leaves no room for data."""
    xknx = xknx_setup
    ia = IndividualAddress("4.0.10")
    ...
    with pytest.raises(ValueError, match=r"leaves no room for memory data"):
        await dmp_user_mem_write_r_co(
            conn, address=0x1000, data=b"\x01", max_apdu_length=4
        )
    await conn.disconnect()


async def test_dmp_user_mem_write_r_co_write_delay(xknx_setup: XKNX) -> None:
    """Test dmp_user_mem_write_r_co sleeps write_delay after each chunk when not verifying."""
    xknx = xknx_setup
    ia = IndividualAddress("4.0.10")
