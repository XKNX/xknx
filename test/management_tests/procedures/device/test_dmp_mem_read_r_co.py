...
async def test_dmp_mem_read_r_co_range_out_of_bounds(xknx_setup: XKNX) -> None:
    """Test dmp_mem_read_r_co raises ValueError when address + size - 1 overflows 0xFFFF."""
    xknx = xknx_setup
    ia = IndividualAddress("4.0.10")

    conn = await xknx.management.connect(ia)
    with pytest.raises(ValueError, match=r"address \+ size - 1 must be <= 0xffff"):
        await dmp_mem_read_r_co(conn, address=0xFFFF, size=2)
    await conn.disconnect()
...
async def test_dmp_mem_read_r_co_error_wrong_address_echoed(xknx_setup: XKNX) -> None:
    """Test dmp_mem_read_r_co raises when a response echoes a different address."""
    xknx = xknx_setup
    ia = IndividualAddress("4.0.10")
