...
async def test_dmp_mem_verify_r_co_mismatch(xknx_setup: XKNX) -> None:
    """Test dmp_mem_verify_r_co raises VerificationError when the device's memory differs."""
    xknx = xknx_setup
    ia = IndividualAddress("4.0.10")
    ...
    responder = asyncio.create_task(respond())
    with pytest.raises(VerificationError, match=r"Memory verify mismatch"):
        await dmp_mem_verify_r_co(conn, address=0x1000, expected_data=b"\x01\x02\x03")
    await responder
...
async def test_dmp_mem_verify_r_co_wrong_address_echoed(xknx_setup: XKNX) -> None:
    """Test dmp_mem_verify_r_co raises when a response echoes a different address."""
    xknx = xknx_setup
    ia = IndividualAddress("4.0.10")
...
async def test_dmp_mem_verify_r_co_range_out_of_bounds(xknx_setup: XKNX) -> None:
    """Test dmp_mem_verify_r_co raises ValueError when address + len(expected_data) - 1 overflows 0xFFFF."""
    xknx = xknx_setup
    ia = IndividualAddress("4.0.10")

    conn = await xknx.management.connect(ia)
    with pytest.raises(
        ValueError, match=r"address \+ len\(expected_data\) - 1 must be <= 0xffff"
    ):
        await dmp_mem_verify_r_co(conn, address=0xFFFF, expected_data=b"\x01\x02")
    await conn.disconnect()
