"""Tests for dmp_ext_run_state_machine_read_r_co_io — KNX v02.01.02 - Management Procedures 03.05.02 - §3.36.4."""
...
from xknx.management.procedures.device.dmp_ext_run_state_machine_read_r_co_io import (
...
def _read_request(ia: IndividualAddress, sequence: int) -> Telegram:
    ...
    return Telegram(
        ...
        payload=apci.FunctionPropertyExtStateRead(
            interface_object_type=343, object_instance=1, property_id=6
        ),
    )
...
async def test_dmp_ext_run_state_machine_read_r_co_io_success() -> None:
...
async def test_dmp_ext_run_state_machine_read_r_co_io_negative_return_code_raises() -> (
    None
):
...
async def test_dmp_ext_run_state_machine_read_r_co_io_unknown_state() -> None:
