...
async def test_dmp_interface_object_scan_r_single_object_single_property(
    xknx_setup: XKNX,
) -> None:
    ...
    async def respond() -> None:
        await _wait_for_request(xknx, 1)
        _prop_desc(
            xknx, ia, seq=0, object_index=0, property_id=1, property_index=0
        )  # outer: exists
        await _wait_for_request(xknx, 2)
        _prop_val(
            xknx, ia, seq=1, object_index=0, property_id=1, data=b"\x00\x01"
        )  # type read
        await _wait_for_request(xknx, 3)
        _prop_desc(
            xknx, ia, seq=2, object_index=0, property_id=78, property_index=0
        )  # inner idx=0
        await _wait_for_request(xknx, 4)
        _prop_desc(
            xknx, ia, seq=3, object_index=0, property_id=0, property_index=1
        )  # inner idx=1: end
        await _wait_for_request(xknx, 5)
        _prop_desc(
            xknx, ia, seq=4, object_index=1, property_id=0, property_index=0
        )  # outer: no more
...
async def test_dmp_interface_object_scan_r_single_object_two_properties(
    xknx_setup: XKNX,
) -> None:
    ...
    async def respond() -> None:
        ...
        await _wait_for_request(xknx, 3)
        _prop_desc(
            xknx, ia, seq=2, object_index=0, property_id=1, property_index=0
        )  # inner idx=0: PID=1
        await _wait_for_request(xknx, 4)
        _prop_desc(
            xknx, ia, seq=3, object_index=0, property_id=78, property_index=1
        )  # inner idx=1: PID=78
        await _wait_for_request(xknx, 5)
        _prop_desc(
            xknx, ia, seq=4, object_index=0, property_id=0, property_index=2
        )  # inner idx=2: end
        await _wait_for_request(xknx, 6)
        _prop_desc(
            xknx, ia, seq=5, object_index=1, property_id=0, property_index=0
        )  # outer: no more
...
async def test_dmp_interface_object_scan_r_multiple_objects(xknx_setup: XKNX) -> None:
    ...
    async def respond() -> None:
        ...
        _prop_desc(
            xknx, ia, seq=8, object_index=1, property_id=0, property_index=2
        )  # end of obj 1 properties
        # no more objects
...
async def test_dmp_interface_object_scan_r_object_type_read_error(
    xknx_setup: XKNX,
) -> None:
