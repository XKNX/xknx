...
KNXnet/IP Core 03.08.02 §5 "Communication Channels" defines the data
...
class ConnectionHeartbeat:
    """
    Keep a KNXnet/IP connection alive - KNXnet/IP Core 03.08.02 §5.4.
    ...
    `(success, status)`. A failed one is repeated three (3) times, as the
    Core specification requires; when those fail as well - or the callable
    raises CommunicationError - the failure is logged, `on_failure` is
    awaited once and the heartbeat ends. What failure means - reconnecting
    a tunnel, closing a device management connection - is the owner's call.
    """

    __slots__ = ("_on_failure", "_send_connectionstate", "_task")

    def __init__(
        self,
        send_connectionstate: Callable[[], Awaitable[tuple[bool, str | None]]],
    ...
    async def _run(self) -> None:
        ...
        while True:
            ...
            try:
                success, status = await self._send_connectionstate()
                if success:
                    continue
                ...
                if success:
                    continue
                logger.warning(
                    "Connection heartbeat failed %s.",
                    "- no response from the server"
                    if status is None
                    else f"with status: {status}",
                )
            except CommunicationError as err:
                logger.warning("Connection heartbeat failed: %s", err)
...
class SequenceVerdict(Enum):
    ...
    OUT_OF_ORDER = auto()  # discard without acknowledgement (E_SEQUENCE_NUMBER §7.3.4)
...
class IncomingSequenceCounter:
    ...
    KNXnet/IP Tunnelling 03.08.04 §2.6.1 states the rules for both ends of a
    tunnel: the expected counter is acknowledged and processed; one less
    than expected is a repetition whose acknowledgement went missing - it is
    acknowledged again but discarded; anything else is discarded without an
    acknowledgement, which has the sender repeat the frame and terminate the
    connection when that is not acknowledged either. Device Management
    03.08.03 §2.3.2 only spells out the server side; a client follows the
    same rules - taking its sentence literally would break the repetition
    it exists for.
