...
class KNXIPDisconnect():
    ...
    def disconnect(self):
        task = asyncio.Task(self.async_disconnect())
        self.xknx.loop.run_until_complete(task)


    @asyncio.coroutine
    def async_disconnect(self):
        print("Requesting disconnect of communiection channel {0}  at {1}:{2}".format(
            self.communication_channel_id,
            self.gateway_ip,
            self.gateway_port))

        yield from self.send_disconnect_request()
    ...
    def response_rec_callback(self, knxipframe):
        if not isinstance(knxipframe.body, DisconnectResponse):
        ...
        if knxipframe.body.status_code == ErrorCode.E_NO_ERROR:
            ...
            print("Error disconnect failed: ", knxipframe.body.status_code)
