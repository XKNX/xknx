...
class RemoteValueColorRGB(RemoteValue):
    ...
    def to_knx(self, value):
        ...
        if not isinstance(value, (list, tuple)):
            raise ConversionError("Could not serialize RemoteValueColorRGB (wrong type)",
                                  value=value, type=type(value))
        if len(value) != 3:
            raise ConversionError("Could not serialize DPT 232.600 (wrong length)",
                                  value=value, type=type(value))
        if any(not isinstance(color, int) for color in value) \
                or any(color < 0 for color in value) \
                or any(color > 255 for color in value):
            raise ConversionError("Could not serialize DPT 232.600 (wrong bytes)", value=value)
